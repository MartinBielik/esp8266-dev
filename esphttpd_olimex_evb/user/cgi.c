...
static char currReleLedState=0;

//Cgi that turns the LED on or off according to the 'led' param in the POST data
int ICACHE_FLASH_ATTR cgiRele(HttpdConnData *connData) {
	int len;
	char buff[1024];
	...
	len=httpdFindArg(connData->postBuff, "ReleLed", buff, sizeof(buff));
	if (len!=0) {
		currReleLedState=atoi(buff);
		ioReleLed(currReleLedState);
	}
